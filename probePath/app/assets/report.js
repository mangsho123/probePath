import { formatMeasurement } from './diagnosis-engine.js';

const root = document.querySelector('#reportApp');
const reportId = new URLSearchParams(window.location.search).get('id');

function renderLoading() {
  root.innerHTML = '<div class="report-shell"><section class="panel"><h1>Loading report...</h1><p>Fetching the saved diagnosis from the local ProbePath server.</p></section></div>';
}
    ...
    <div class="report-shell">
      ...
      <section class="panel">
        ...
        <div class="report-list">
          ${payload.report.measurements.map((measurement) => `<article class="report-row"><strong>${measurement.title}</strong><span>${measurement.formattedValue || formatMeasurement(measurement.value, measurement.unit)}</span><small>${measurement.status}. ${measurement.detail}</small></article>`).join('')}
        </div>
      </section>
      <section class="panel">
